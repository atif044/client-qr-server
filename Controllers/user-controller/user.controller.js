...
exports.changePassword=catchAsyncErrors(async(req,res,next)=>{
  ...
  try {
    let [rows,fields]=await db.query('select * from users where email= ?',[email]);
    if(rows.length===0){
      return next(new ErrorHandler("Your Account doesnt exist. Please make a new Account",400));
    }
    ...
    if(oldPassword===newPassword){
      return next(new ErrorHandler("New Password can not be same as old password",400));
    }
    const hashed=await hash(newPassword,10);
    const results=await db.query('Update users set password = ? where email = ?',[hashed,email]);
...
exports.resetPassword=catchAsyncErrors(async(req,res,next)=>{
  ...
  try {
    ...
    let searchIfAlreadyRequested=await db.query("Select * from resetpasswordtokens where email = ?",email);
    if(searchIfAlreadyRequested[0].length>=1){
      let isDeleted=await db.query("Delete from resetpasswordtokens where email = ?",email);
...
exports.verifyResetPasswordToken=catchAsyncErrors(async(req,res,next)=>{
  ...
  try {
    ...
    let changedPassword=await db.query("Update users set password = ? where email = ?",[hashedPassword,result[0][0].email]);
    if(changedPassword[0].affectedRows===0){
      await db.query("ROLLBACK")
      return next(new ErrorHandler("An Error Occurred",400));
    }
    await db.query("DELETE FROM resetpasswordtokens where tokenvalue = ?",[token]);
    await db.query("COMMIT");
...
exports.addShippingAddress=catchAsyncErrors(async(req,res,next)=>{
  ...
  try {
    let query=await db.query("select count(Country) as 'count' from shipping_address where email = ?",[email]);
    if(query[0][0].count==3){
      return next(
        new ErrorHandler("Can't Add More than 3 Addresses",400)
      );
    }
    let result=await db.query("insert into shipping_address(Country,City,Address1,Address2,phoneNo,email,userChoiceEmail) values (?,?,?,?,?,?,?)",[
      country,city,address1,address2,phoneNo,email,dataEmail
    ])
    if(result[0].affectedRows===0){
      return next(new ErrorHandler("An Error Occurred",400));
    }
    res.status(201).json({status:"success",message:"Shipping Address Added Successfully"})
  } catch (error) {
    return next(
      new ErrorHandler(error.message, error.code || error.statusCode)
    );
...
exports.removeShippingAddress=catchAsyncErrors(async(req,res,next)=>{
  ...
  let email=req.userData.user.email;
  try {
    let result=await db.query("delete from shipping_address where id= ? and email = ?",[
      id,email
    ])
    if(result[0].affectedRows===0){
      return next(new ErrorHandler("An Error Occurred",400));
    }
    res.status(201).json({status:"success",message:"Shipping Address removed Successfully"})
  } catch (error) {
    if(error.code==="ER_ROW_IS_REFERENCED_2"){
      return next(new ErrorHandler("You have an existing order on this id! You can't delete it",400))
    }
    return next(
      new ErrorHandler(error.message, error.code || error.statusCode)
    );
...
exports.MyProfile=catchAsyncErrors(async(req,res,next)=>{
  ...
  try {
    ...
    let shippingAddresses=await db.query("Select * from shipping_address where email = ?",[email])
    let Data={
...
exports.fetchPhotos=catchAsyncErrors(async(req,res,next)=>{
  ...
  let email=req.userData.user.email;
  try {
    let result=await db.query("select * from media where email = ? and mediatype = ?",[email,type]);
    if(result[0].length===0){
      return next(new ErrorHandler("No Images to display"));      
    }
    let dataLink=result[0].map(({datalink,publicid})=>({datalink:datalink,publicid:publicid}))
        return res.status(200).json({status:"success",message:"Images found",body:dataLink})
...
exports.fetchVideos=catchAsyncErrors(async(req,res,next)=>{
  ...
  try {
    ...
    let dataLink=result[0].map(({datalink,publicid})=>({datalink:datalink,publicid:publicid}))
    return res.status(200).json({status:"success",message:"Videos found",body:dataLink})
  } catch (error) {
    return next(
      new ErrorHandler(error.message, error.code || error.statusCode)
    );
...
exports.fetchShippingAddresses=catchAsyncErrors(async(req,res,next)=>{
  let email=req.userData.user.email;
  try {
    let result=await db.query("Select * from shipping_address where email = ?",[email]);
    if(result[0].length===0){
      return next(new ErrorHandler("No Shipping Address Found. Add One Please",400))
    }
    return res.status(200).json({status:"success",message:"Shipping Addresses", body:result[0]})
...
exports.fetchMyOrders=catchAsyncErrors(async(req,res,next)=>{
  let email=req.userData.user.email;
  try {
    let result=await db.query("Select * from orders where email = ?",[email]);
    let result2=await db.query("select * from qrcode_orders where email = ?",[email]);
    let body=result[0].concat(result2[0]);
    
    return res.status(200).json({status:"success",message:"My Confirmed Orders",body:body})
  } catch (error) {
    return next(new ErrorHandler(error.message, error.code || error.statusCode))
  }
});
...
let result=await db.query("Select paymentarrived from users where email = ?",[email]);
if(result[0].length===0){
  return next( new ErrorHandler("Account doesn't exist",400));
}
...
let videos=await db.query("select * from media where email = ? and mediatype = ? ",[email,"video"]);
let images=await db.query("select * from media where email = ? and mediatype = ? ",[email,"image"]);
...
return res.status(200).json({
  ...
  body:{
  videos:videos[0].map(({datalink})=>datalink),
  images:images[0].map(({datalink})=>datalink)
  }
})
